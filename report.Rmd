...
First, we noticed a few NAs in the data set and decided to explore more. As we can see below, there are quite a few NAs in the Revenue column. Since this variable is important for our definition of success, we removed all rows that contained any NAs.
...
actorRevenue = head(actorFinal[order(-actorFinal$Revenue),], 10)
...
ggplot(actorRevTrim) + geom_bar(aes(x = Actor, y = Revenue, fill = Revenue), stat = "identity") + coord_flip() + geom_text(aes(x=Actor,y = Revenue, label = paste(count, "movies")), color = "white", size = 5, position = position_stack(vjust = 0.5)) + labs(title = "Average Revenue by Actor")

actorMetascore = head(actorFinal[order(-actorFinal$Metascore),], 10)
...
directorRating = head(directorFinal[order(-directorFinal$Rating),], 10)
...
Ashley: In this project, we decided to separate the data analysis section and we all focused on different questions of interest we came up together as a team. I specifically analyzed how genres of a movie contribute to it's success, which was the last question analyzed above. I also organized the sections in our report document so we could be sure to follow the rubric and guidelines, as well as help set up our presentation. 
