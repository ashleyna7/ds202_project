...
ggplot(movie2, aes(x = Runtime, y = mean_rev, color=group)) + geom_point() + labs(title = "Average Revenue of Movies for each Runtime") + xlab("Movie Runtime (min)") + ylab("Revenue (mil)")
...
ggplot(movie2, aes(x = Runtime, y = mean_rat, colour=group)) + geom_point() + labs(title = "Average Rating of Movies for each Runtime") + xlab("Movie Runtime (min)") + ylab("Rating") + geom_smooth(method="lm", se = FALSE, fullrange=TRUE) 

movie2 = movie %>%
    group_by(Runtime) %>%
    summarize(mean_met = mean(Metascore))

ggplot(movie2, aes(x = Runtime, y = mean_met)) + geom_point() + labs(title = "Average Metascore of Movies for each Runtime") + xlab("Movie Runtime (min)") + ylab("Metascore")
